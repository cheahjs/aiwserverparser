...
namespace Server_Parser_3
{
    ...
    /*public class XUID
    {
        ...
        protected internal static extern bool SetupDiDestroyDeviceInfoList(IntPtr hDevInfo);
        [DllImport("setupapi.dll")]
        static extern Boolean SetupDiGetDeviceRegistryPropertyA(IntPtr DeviceInfoSet,
        ...
        [Flags]
        protected internal enum DIGCF : int
        {
        ...
        protected internal enum SPDRP : int
        {
        ...
        protected internal bool connectedInterface = true;

        protected internal uint GetSteamID()
        {
            if (steamID == 0)
            {
                String dbg = "";
                ...
                try
                {
                    ...
                    foreach (NetworkInterface iface in ifaces)
                    {
                        ...
                        {
                            if (!IsValidInterface(iface.Id, false))
                            {
                            ...
                            var address = iface.GetPhysicalAddress().GetAddressBytes();

                            try
                            {
                                steamID = hash(address, address.Length);
        ...
        protected internal unsafe uint hash(byte[] inpStr, int len)
        {
            uint value = 0, temp = 0;
            for (int i = 0; i < len; i++)
            {
        ...
        protected internal unsafe Microsoft.Win32.RegistryKey GetNetworkRegistryKey(String id)
        {
            try
            {
                ...
                string[] keyNames = networkInterfaceKey.GetSubKeyNames();

                foreach (String keyName in keyNames)
                {
                    Microsoft.Win32.RegistryKey key = networkInterfaceKey.OpenSubKey(keyName, true);
        ...
        unsafe String GetDeviceIDForDriverKey(String key)
        {
            int index = 0;
            char[] buffer = new char[1024];
            String deviceID = "";
            Guid GUID_DEVCLASS_NET = new Guid("{0x4d36e972, 0xe325, 0x11ce, {0xbf, 0xc1, 0x08, 0x00, 0x2b, 0xe1, 0x03, 0x18}}");

            SP_DEVINFO_DATA data = new SP_DEVINFO_DATA();
            data.cbSize = (uint)Marshal.SizeOf(data);

            IntPtr handle = SetupDiGetClassDevs(ref GUID_DEVCLASS_NET, null, IntPtr.Zero, (uint)DIGCF.DIGCF_PRESENT);

            while (SetupDiEnumDeviceInfo(handle, (uint)index, ref data))
            {
                index++;

                if (SetupDiGetDeviceRegistryPropertyA(handle, data, (uint)SPDRP.SPDRP_DRIVER, 0, new StringBuilder(buffer.ToString()), 1024, IntPtr.Zero))//(uint)Marshal.SizeOf(buffer), IntPtr.Zero))
                {
                    String key2 = new String(buffer);

                    if (key.Replace("HKEY_LOCAL_MACHINE\\", "").ToLower() == key2.ToLower())
                    {
                        if (SetupDiGetDeviceRegistryPropertyA(handle, data, (uint)SPDRP.SPDRP_HARDWAREID, 0, new StringBuilder(buffer.ToString()), (uint)Marshal.SizeOf(buffer), IntPtr.Zero))
                        {
                            deviceID = new String(buffer);
                        }
                    }
                }
            }

            if (Convert.ToBoolean((uint)handle))
            {
        ...
        unsafe bool IsValidInterface(String id, bool legacy)
        {
            ...
            String deviceID3 = (String)key.GetValue("MatchingDeviceId", "");
            ...
            return (deviceID.ToLower().StartsWith("pci") || deviceID.ToLower().StartsWith("usb") || deviceID.ToLower().StartsWith("{") || deviceID2.ToLower().StartsWith("pci") || deviceID2.ToLower().StartsWith("usb") || deviceID3.ToLower().StartsWith("pci") || deviceID3.ToLower().StartsWith("usb") || deviceID3.ToLower().StartsWith("{"));
        }
        ...
        unsafe bool IsConnectedInterface(String id)
        {
            if (Environment.OSVersion.Platform != PlatformID.Win32Windows && Environment.OSVersion.Platform != PlatformID.Win32NT)
            {
                return true;
